    ...
    while (true) {
        ...
        switch (i) {
            ...
            case INS_SIMPLE_OPTIONAL_TO_BOX_REMOTE: {
                EmojicodeInstruction typeId = thread->consumeInstruction();
                auto size = thread->consumeInstruction();
                auto *src = thread->popOpr(size);

                if (src->raw != T_NOTHINGNESS) {
                    auto *object = newArray((size + 1) * sizeof(Value));
                    std::memcpy(object->val<Value>(), src, size * sizeof(Value));
            ...
            case INS_BOX_PRODUCE: {
                thread->pushOpr(thread->consumeInstruction());
                continue;
            }
            ...
            case INS_IS_NOTHINGNESS:
                thread->pushOpr(thread->popOpr().raw == T_NOTHINGNESS);
                continue;
            case INS_IS_ERROR:
                thread->pushOpr(thread->popOpr().raw == T_NOTHINGNESS);
                continue;
